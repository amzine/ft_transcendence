...
const PLAYGROUND_SIZE = {x : 1200, y: 720};
const BALL_SIZE = 40;
...
const RIGHT_PADDLE_POSITION = {
    x : PLAYGROUND_SIZE.x - PADDLE_SIZE.x - PADDLE_SPACING,
    y : PLAYGROUND_SIZE.y / 2 - PADDLE_SIZE.y / 2,
};
...
class Character{
    ...
    constructor(
        ...
        this.position = {
            x: postion.x + getRandomInt(-waste.x , waste.x),
            y: postion.y + getRandomInt(-waste.y , waste.y),
        };
    }
...
export class Pong{
	...
	private particles = [] as Character[];
	private score = [0, 0];
	private timer = 0;
	private paddleHit = '';
    ...
    updatePaddleHit(side : 'left' | 'right'){
        this.paddleHit = side;
    }
